# Databricks notebook source exported at Thu, 15 Sep 2016 18:17:37 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWordsCount = wordCountsDF.select(wordCountsDF.word).count()
...
shakeWordsDF = shakespeareDF.select(explode(split('sentence',' ')).alias('word')).where(length('word')>0)
shakeWordsDF.show()
...
#topWordsAndCountsDF = wordCount(shakeWordsDF).orderBy('count')
